...
namespace uBuilder
{
    public class Server
    {
        ...
        public string serverName = "uBuilder Server";
        public string motd = "Welcome to my uBuilder Server!";
        ...
        public void LoadConfig()
        {
            try
            {
                ...
                else
                {
                    ...
                    cfgWriter.WriteLine("#uBuilder Configuration File");
                    cfgWriter.WriteLine("server-name = uBuilder Server");
                    cfgWriter.WriteLine("server-motd = Welcome to my uBuilder server");
        ...
        {
            if (!File.Exists("players.stat")) return;
            StreamReader pInfo = new StreamReader(File.Open("players.stat", FileMode.Open));
            while (!pInfo.EndOfStream)
            {
                string line = pInfo.ReadLine();
                if (line.Contains("{"))
                {
                    string name = line.Substring(0, line.IndexOf("{")).Trim();
                    Account pAcc = new Account(name.ToLower());
                    string temp = "";
                    while (!temp.Contains("}"))
                    {
                        temp = pInfo.ReadLine();
                        ...
                        {
                            case "visitcount":
                                pAcc.visitcount = Int32.Parse(args[1]);
                                break;
                            case "lastjoined":
                                pAcc.lastjoined = args[1];
                                break;
                            case "blockscreated":
                                pAcc.SetBlockStats(true, Int32.Parse(args[1]));
                                break;
                            case "blocksdestroyed":
                                pAcc.SetBlockStats(false, Int32.Parse(args[1]));
                                break;
                            case "messagessent":
                                pAcc.messagesSent = Int32.Parse(args[1]);
                                break;
                            case "lastseenip":
                                pAcc.SetIP(args[1]);
                                break;
                    ...
                    accounts.Add(name, pAcc);
                }
            }

            pInfo.Close();
            logger.log("Loaded account statistics");
        }

        public void SavePlayerStats()
        {
            StreamWriter pInfo = new StreamWriter(File.Open("players.stat", FileMode.Create));
            foreach (KeyValuePair<string, Account> pAcc in accounts)
            {
                pInfo.WriteLine(pAcc.Key.ToLower() + " {");

                pInfo.WriteLine("    visitcount=" + pAcc.Value.visitcount);
                pInfo.WriteLine("    lastjoined=" + pAcc.Value.lastjoined);
                pInfo.WriteLine("    blockscreated=" + pAcc.Value.blocksCreated);
                pInfo.WriteLine("    blocksdestroyed=" + pAcc.Value.blocksDestroyed);
                pInfo.WriteLine("    messagessent=" + pAcc.Value.messagesSent);
                pInfo.WriteLine("    lastseenip=" + pAcc.Value.lastseenip);
